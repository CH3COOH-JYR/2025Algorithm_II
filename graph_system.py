...
class Graph:
    ...
    def show(self, title="Graph Visualization", figsize=(12, 8)):
        """
        图可视化
        ...
        if self.G.number_of_nodes() > 1000:
            print(f"图太大({self.G.number_of_nodes()}节点)，使用子采样可视化...")
            # 随机采样1000个节点
            sampled_nodes = np.random.choice(list(self.G.nodes()), 
                                           min(1000, self.G.number_of_nodes()), 
                                           replace=False)
    ...
    def show_coreness(self, core_numbers=None, figsize=(12, 8)):
        """
        可视化k-core结构
        
        Args:
            core_numbers: k-core分解结果
            figsize: 图片大小
        """
        ...
        if self.G.number_of_nodes() > 1000:
            ...
            sampled_nodes = np.random.choice(list(self.G.nodes()), 
                                           ...
                                           replace=False)
            subgraph = self.G.subgraph(sampled_nodes)
            sub_core_numbers = {node: core_numbers[node] for node in sampled_nodes}
        else:
            subgraph = self.G
            sub_core_numbers = core_numbers
        
        # 计算布局
        pos = nx.spring_layout(subgraph, k=1, iterations=50)
        
        # 根据core number设置颜色
        max_core = max(sub_core_numbers.values()) if sub_core_numbers else 1
        colors = [sub_core_numbers[node] / max_core for node in subgraph.nodes()]
        
        # 绘制
        nx.draw_networkx_nodes(subgraph, pos, node_color=colors, 
                              cmap=plt.cm.viridis, node_size=30, alpha=0.8)
        nx.draw_networkx_edges(subgraph, pos, alpha=0.3, width=0.5)
        
        plt.title("K-Core Structure Visualization")
    ...
    for dataset in datasets:
        ...
        try:
            ...
            if g.G.number_of_nodes() <= 5000:
                print(f"\n6. 图可视化")
                g.show(f"{dataset_name} - Original Graph")
                
                core_numbers = nx.core_number(g.G)
                g.show_coreness(core_numbers, f"{dataset_name} - K-Core Structure")
            else:
                print(f"\n图太大，跳过可视化")
